package com.infraflow.healing.config;

import org.apache.kafka.clients.admin.NewTopic;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.kafka.config.TopicBuilder;

@Configuration
public class HealingEngineConfig {

    @Value("${infraflow.kafka.topic.pipeline-events}")
    private String pipelineEventsTopic;

    @Value("${infraflow.kafka.topic.healing-events}")
    private String healingEventsTopic;

    @Bean
    public NewTopic pipelineEventsTopic() {
        return TopicBuilder.name(pipelineEventsTopic)
                .partitions(3)
                .replicas(1)
                .build();
    }

    @Bean
    public NewTopic healingEventsTopic() {
        return TopicBuilder.name(healingEventsTopic)
                .partitions(3)
                .replicas(1)
                .build();
    }
}
